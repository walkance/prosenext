...
import {
  ...
  TextSelection,
  NodeSelection,
  SelectionRange as selection_range,
} from 'prosemirror-state';
import {Decoration, DecorationSet} from 'prosemirror-view';
import {Fragment, ResolvedPos, Slice} from 'prosemirror-model';

const SelectionRange = selection_range as any;
...
export class CellSelection extends Selection {
  ...
  constructor(public $anchorCell: ResolvedPos, public $headCell = $anchorCell) {
    ...
    // console.log(ranges[0].$from, ranges[0].$to, ranges)
  ...
  static colSelection($anchorCell, $headCell = $anchorCell) {
    const map = ProsemirrorTable.get($anchorCell.node(-1)),
      start = $anchorCell.start(-1);
  ...
  // right of the table.
  isRowSelection() {
    const map = ProsemirrorTable.get(this.$anchorCell.node(-1)),
      start = this.$anchorCell.start(-1);
...
class CellBookmark {
  constructor(private anchor, private head) {
    this.anchor = anchor;
    this.head = head;
  }
...
export function normalizeSelection(state, tr, allowTableNodeSelection) {
  ...
  if (sel instanceof NodeSelection && (role = sel.node.type.spec.tableRole)) {
    if (role == 'cell' || role == 'header_cell') {
      ...
      const map = ProsemirrorTable.get(sel.node),
        start = sel.from + 1;
