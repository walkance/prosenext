...
export class ProsemirrorTable {
  ...
  // вроде это получения номера столбца
  colCount(pos: number) {
    for (let i = 0; i < this.map.length; i++)
  ...
  //  получения номера строки
  rowCount(pos: number) {
    for (let i = 0; i < this.map.length; i++)
      if (this.map[i] == pos) {
        // console.log('pos, this.map', pos, this.map, i);
  ...
  static get(table: Node): ProsemirrorTable {
...
function computeMap(table) {
  ...
  const tableMap = new ProsemirrorTable(width, height, map, problems);
  let badWidths = false;
