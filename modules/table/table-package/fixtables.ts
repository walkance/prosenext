...
export function fixTable(state: EditorState, table, tablePos, tr: Transaction) {
  const map = ProsemirrorTable.get(table);
  ...
  for (let i = 0; i < map.problems.length; i++) {
    ...
    if (prob.type == 'collision') {
      ...
      tr.setNodeMarkup(
        tr.mapping.map(tablePos + 1 + prob.pos),
        null,
        setAttr(cell.attrs, 'rowheight', prob.rowheight)
      );
    }
  }
  let first, last;
